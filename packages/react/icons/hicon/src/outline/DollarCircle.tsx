import * as React from "react";
import type { SVGProps } from "react";
const SvgDollarCircle = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    {...props}
  >
    <path
      fill="currentColor"
      fillRule="evenodd"
      d="M12.75 5a.75.75 0 0 0-1.5 0v.308a3.7 3.7 0 0 0-.953.386c-2.722 1.572-2.376 5.605.574 6.69l.379.14v4.695a2.2 2.2 0 0 1-1.312-1.02l-.376-.65a.75.75 0 1 0-1.299.75l.376.65a3.7 3.7 0 0 0 2.611 1.804V19a.75.75 0 0 0 1.5 0v-.308a3.7 3.7 0 0 0 .953-.386c2.722-1.572 2.376-5.605-.574-6.69l-.379-.14V6.782a2.2 2.2 0 0 1 1.312 1.02l.376.65a.75.75 0 0 0 1.299-.75l-.376-.65a3.7 3.7 0 0 0-2.611-1.805zm-1.703 1.993q.1-.058.203-.103v4.03c-1.627-.719-1.78-3.016-.203-3.927m1.906 10.014a2 2 0 0 1-.203.103v-4.03c1.627.718 1.78 3.016.203 3.927"
      clipRule="evenodd"
    />
    <path
      fill="currentColor"
      fillRule="evenodd"
      d="M12 1.25C6.063 1.25 1.25 6.063 1.25 12S6.063 22.75 12 22.75 22.75 17.937 22.75 12 17.937 1.25 12 1.25M2.75 12a9.25 9.25 0 1 1 18.5 0 9.25 9.25 0 0 1-18.5 0"
      clipRule="evenodd"
    />
  </svg>
);
export default SvgDollarCircle;
